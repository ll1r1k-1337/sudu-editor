...
public class JsInlineDiff implements JsInlineDiffView {

  public final WebWindow window;
  private UnifiedDiffWindow w;
  private JsInlineDiffViewController controller;
  private IDiffSizeChangeCallback sizeListener;
  private int eventNumLines, eventLineHeight;
  private float eventCssLineHeight;

  public JsInlineDiff(
      UnifiedDiffWindow inlineDiff, WebWindow ww,
      EditArgs args
  ) {
    window = ww;
    w = inlineDiff;
    controller = new JsInlineDiffViewController0(w);
  ...
  void onDocumentSizeChange() {
    if (sizeListener != null && w.rootView.dpr != 0) {
      int numLines = w.rootView.getNumLines();
      int lineHeight = w.rootView.lineHeight();
  ...
  public static JsInlineDiffView newInlineDiff(EditArgs arguments) {
    ...
    var inlineDiff = new InlineDiff(w.api(), arguments.getDisableParserOrDefault());
    w.setScene(inlineDiff);
    return new JsInlineDiff(inlineDiff.w, w, arguments);
  }
}
