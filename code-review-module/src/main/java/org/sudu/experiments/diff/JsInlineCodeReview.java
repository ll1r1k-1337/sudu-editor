...
public class JsInlineCodeReview implements JsCodeReviewView {

  public final WebWindow window;
  private InlineDiffWindow w;
  private JsFileDiffViewController controller;
  private JsCodeReviewView.IDiffSizeChangeCallback sizeListener;
  private int eventNumLines, eventLineHeight;
  private float eventCssLineHeight;

  public JsInlineCodeReview(
      InlineDiffWindow inline, WebWindow ww,
      EditArgs args
  ) {
    this.window = ww;
    this.w = inline;
    controller = new JsInlineFileDiffViewController0(w);
  ...
  void onDocumentSizeChange() {
    if (sizeListener != null && w.rootView.dpr != 0) {
      int numLines = w.rootView.editor.getNumLines();
      int lineHeight = w.rootView.editor.lineHeight();
  ...
  public static JsCodeReviewView newInlineCodeReview(EditArgs arguments) {
    ...
    var inline = new InlineFileDiff(w.api(), arguments.getDisableParserOrDefault());
    w.setScene(inline);
    return new JsInlineCodeReview(inline.w, w, arguments);
  }
}
